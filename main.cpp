...
//    cout << c << '\n';
    ...
    auto cycles = game.find_cycles(1ull << 16, 1);
    ...
    for(const auto& cycle : cycles)
    {
        ...
        {
            size_t index_in_p_matrix = cycle_indices[dest_cycle] + cycle_indices[cycle] * cycles.size();
            auto m = static_cast<double>(dest_frequency);
            auto n = static_cast<double>(dest_cycle.frames().size() * Frame<s>::CellCount);
